...
const TourBookingsTable = () => {
    ...
    const fetchingApi = async () => {
        let apiUrl = `${host[0].hostUrl}/api/get/data/bookTour`;
    ...
    const deleteRow = async (id) => {
        let apiUrl = `${host[0].hostUrl}/api/delete/data/bookTour/row/by/Id`;
    ...
    const fetchingTourPackagesApi = async () => {
        ...
        try {
            ...
            // console.log(response);
            if (response.status == 200) {
                setTourPackagesData(response.data.data);
            }
        } catch (error) {
            // console.log("Something went wrong while fetching api: ", error);
        }
    ...
    const onChangeHandler = (e) => {
        ...
        if (inputValue.trim() == '') {
            ...
            const tour = tourPackagesData.find((data) => (data.desertSafariName.toLowerCase().includes(inputValue)));

            const filterData = data.filter(({ id, transferType, pickupDate, totalAmount, discountOffer, afterDiscount, }) => (id.toString().includes(inputValue) || transferType.toLowerCase().includes(inputValue) || pickupDate.toLowerCase().includes(inputValue) || totalAmount.toLowerCase().includes(inputValue) || discountOffer.toLowerCase().includes(inputValue) || afterDiscount.toLowerCase().includes(inputValue)));
            if (filterData.length > 0) {
                setFilterData(filterData);
            }
            else if (tour) {
                const filterDataByTour = data.filter(({ tourId }) => (tourId == tour.id));
                if (filterDataByTour.length > 0) {
                    setFilterData(filterDataByTour);
                }
                else {
                    setFilterData([]);
                }
    ...
    return (
        <>
            <div className="page-header">
                <div>
                    <h1 className="page-title">Tour Bookings Data</h1>
                </div>
                    ...
                    <div className="col-lg-12">
                        <div className="card">
                            <div className="card-header border-bottom" style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <h3 className="card-title">Tour Bookings Data List</h3>
                                {
                                    data.length > 0 ?
                                                        ...
                                                        currentData.map((item, index) => {
                                                            return (
                                                                <tr data-id="1" key={index}>
                                                                    ...
                                                                    <td data-field="name">
                                                                        {
                                                                            tourPackagesData.length > 0 ?
                                                                                tourPackagesData.map((data, key) => {
                                                                                    return (
                                                                                        <React.Fragment key={key}>{item.tourId == data.id ? data.desertSafariName : ''}</React.Fragment>
                                                                                    )
                                                                                })
                                                                                :
                                                                                item.tourId
                                                                    ...
                                                                    <td data-field="age">{item.transferType}</td>
                                                                    <td data-field="age">{item.pickupDate}</td>
                                                                    <td data-field="age">{item.adult}</td>
                                                                    <td data-field="age">{item.child}</td>
                                                                    <td data-field="age">{item.infant}</td>
                                                                    <td data-field="age">{item.totalAmount} AED</td>
                                                                    <td data-field="age">{item.discountOffer}</td>
                                                                    <td data-field="age">{item.afterDiscount} AED</td>
                                                                    <td style={{ width: 80 }}>
                                                                        <a
