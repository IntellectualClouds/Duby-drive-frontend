...
const ContactFormTable = () => {
  ...
  const fetchingApi = async () => {
    let apiUrl = `${host[0].hostUrl}/api/get/data/contactForm`;
  ...
  const deleteRow = async (id) => {
    let apiUrl = `${host[0].hostUrl}/api/delete/data/contactForm/row/by/Id`;
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const filterData = data.filter(({ id, customerName, customerEmail, customerPhoneNumber }) => (id.toString().includes(inputValue) || customerName.toLowerCase().includes(inputValue) || customerEmail.toLowerCase().includes(inputValue) || customerPhoneNumber.toLowerCase().includes(inputValue)));
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <h1 className="page-title">Contact Form Data</h1>
        </div>
          ...
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header border-bottom" style={{ display: 'flex', justifyContent: 'space-between' }}>
                <h3 className="card-title">Contact Form Data List</h3>
                {
                  data.length > 0 ?
                            ...
                            currentData.map((item, index) => {
                              return (
                                <tr data-id="1" key={index}>
                                  ...
                                  <td data-field="age">{item.customerMessage}</td>
                                  <td style={{ width: 80 }}>
                                    <a
