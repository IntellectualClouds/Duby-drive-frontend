...
const DriverServiceFormTable = () => {
    ...
    const fetchingApi = async () => {
        let apiUrl = `${host[0].hostUrl}/api/get/data/driverService`;
    ...
    const fetchingSafeDriversApi = async () => {
        ...
        try {
            ...
            // console.log(response);
            if (response.status == 200) {
                setAllDriversData(response.data.data);
            }
        } catch (error) {
            // console.log("Something went wrong while fetching api: ", error);
        }
    ...
    const deleteRow = async (id) => {
        let apiUrl = `${host[0].hostUrl}/api/delete/data/driverService/row/by/Id`;
    ...
    const onChangeHandler = (e) => {
        ...
        if (inputValue.trim() == '') {
            ...
            const filterData = data.filter(({ id, customerName, customerEmail, customerPhoneNumber }) => (customerName.toLowerCase().includes(inputValue) || id.toString().includes(inputValue) || customerEmail.toLowerCase().includes(inputValue) || customerPhoneNumber.toLowerCase().includes(inputValue)));
    ...
    return (
        <>
            <div className="page-header">
                <div>
                    <h1 className="page-title">Driver Service Data</h1>
                </div>
                    ...
                    <div className="col-lg-12">
                        <div className="card">
                            <div className="card-header border-bottom" style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <h3 className="card-title">Driver Service Data List</h3>
                                {
                                    data.length > 0 ?
                                                        ...
                                                        currentData.map((item, index) => {
                                                            return (
                                                                <tr data-id="1" key={index}>
                                                                    ...
                                                                    <td data-field="age">{allDriversData.map((itm, ind) => {
                                                                        return (
                                                                            <React.Fragment key={ind}>{item.serviceType == itm.id ? `${itm.servicePeriod} ${itm.serviceType} ${itm.carWithDriver}` : ''}</React.Fragment>
                                                                        )
                                                                    })}</td>
                                                                    <td style={{ width: 80 }}>
                                                                        <a
...
export default DriverServiceFormTable;
