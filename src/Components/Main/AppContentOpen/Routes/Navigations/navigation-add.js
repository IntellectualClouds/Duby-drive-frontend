...
const NavigationForm = () => {
    ...
    const apiCall = async (navigationName, path, type, status) => {
        let apiUrl = `${host[0].hostUrl}/api/post/data/navigations`;

        try {
            let response = await axios({
                method: "POST",
                url: apiUrl,
                data: { navigationName, path, type, status },
            });
            // console.log(response);
            if (response.status == 200) {
                if (file !== null) {
    ...
    const addNewNavigation = async (e) => {
        e.preventDefault();
        if (navigationName.trim() == "" || path.trim() == "" || type.trim() == "") {
            Swal.fire({
                position: "center",
                icon: "info",
                title: "Cannot process your entry!",
                text: "Please enter navigation name and path / link, Please enter correct details",
                showConfirmButton: true,
                timer: 3000,
                customClass: {
            ...
            setBtnDisable(true);
            apiCall(navigationName.trim(), path.trim(), type.trim(), statusValue);
        }
    };

    useEffect(() => {
        setType("FRONTEND FORMS DATA");
    }, []);
                ...
                <div className="col-lg-12 col-md-12">
                    <div className="card">
                        <div className="card-header border-bottom">
                            <h3 className="card-title">Add New Navigations</h3>
                        </div>
                        <div className="card-body">
                            ...
                            >
                                <div className="d-flex">
                                    ...
                                    <div className="col-md-3">
                                        ...
                                        {path.trim().length < 2 ? (
                                            ...
                                            >
                                                Please enter navigation path or link
                                            </label>
                                        ) : (
                                            <label
                                                htmlFor="validationCustom01"
                                ...
                                <div className="col-12">
                                    ...
                                    <button className="btn btn-primary" onClick={addNewNavigation} disabled={btnDisable}>
                                        {
                                            btnDisable ?
                                                <>
