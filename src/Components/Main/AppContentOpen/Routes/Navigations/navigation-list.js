...
const NavigationTable = () => {
    ...
    //! Fetching Car Types List Api...
    const fetchingApi = async () => {
        let apiUrl = `${host[0].hostUrl}/api/get/data/navigations`;
    ...
    const deleteRow = async (id, navIcon) => {
        // console.log(`Item ID: ${id}`);

        //! Calling Api for deleting Row By ID...
        let apiUrl = `${host[0].hostUrl}/api/delete/data/navigation/row/byId`;
    ...
    const onChangeHandler = (e) => {
        ...
        if (inputValue.trim() == '') {
            ...
            const filterData = data.filter(({ id, navigation }) => (navigation.toLowerCase().includes(inputValue) || id.toString().includes(inputValue)));
        ...
        <>
            ...
            >
                <div className="btn btn-primary" style={{ marginBottom: 10 }}>
                    <Link to={"/navigationForm"} style={{ color: "white" }}>
                        Add New Navigation
                    </Link>
                </div>
            </div>
                    ...
                    <div className="col-lg-12">
                        <div className="card">
                            <div className="card-header border-bottom" style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <h3 className="card-title">Navigations List</h3>
                                {
                                    data.length > 0 ?
                                        <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                                            <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search Navigation..." type="search" style={{ paddingRight: 40 }} />
                                                        ...
                                                        currentData.map((item, index) => {
                                                            return (
                                                                <tr data-id="1" key={index}>
                                                                    ...
                                                                    <td data-field="name">{item.navigationName}</td>
                                                                    <td data-field="age"><Link to={item.path}>{item.path}</Link></td>
                                                                    <td data-field="age">
                                                                        <img
                                                                            src={`${host[0].hostUrl}/icons/side-nav-icons/${item.navIcon}`}
                                                                            alt={item.navIcon}
                                                                            onError={(e) => {
                                                                    ...
                                                                    <td style={{ width: 120 }}>
                                                                        ...
                                                                        <a
                                                                            ...
                                                                            style={{ marginLeft: 10 }}
                                                                            onClick={() => {
                                                                                deleteRow(item.id, item.navIcon);
                                                                            }}
                                                                        >
                                                    ...
                                                    filterData.length > 0 ?
                                                        filterData.map((item, index) => {
                                                            return (
                                                                <tr data-id="1" key={index}>
                                                                    <td data-field="id">#{item.id}</td>
                                                                    <td data-field="name">{item.navigation}</td>
                                                                    <td data-field="age">{item.path}</td>
                                                                    <td data-field="age">
                                                                        <img
                                                                            src={`${host[0].hostUrl}/icons/side-nav-icons/${item.navIcon}`}
                                                                            alt={item.navIcon}
