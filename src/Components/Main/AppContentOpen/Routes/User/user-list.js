...
//! Importing porfile image for error image...
import profileImageError from "../../../../../assets/icons/profileImageError.png";
...
const UserTable = () => {
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const filterData = data.filter(({ id, userName, userEmail, userContactNumber, userRole }) => (id.toString().includes(inputValue) || userName.toLowerCase().includes(inputValue) || userEmail.toLowerCase().includes(inputValue) || userContactNumber.toLowerCase().includes(inputValue) || userRole.toLowerCase().includes(inputValue)));
                ...
                <h3 className="card-title">User List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search User..." type="search" style={{ paddingRight: 40 }} />
                            ...
                            currentData.map((item, index) => {
                              return (
                                <tr data-id="1" key={index}>
                                  ...
                                  <td data-field="age">
                                    <img
                                      src={`${host[0].hostUrl}/profile/profile-pictures/${item.pictureUrl}`}
                                      alt={item.pictureUrl}
                                      onError={(e) => {
                                        e.target.src = profileImageError;
                                      }}
                                      height={50}
                                      width={50}
