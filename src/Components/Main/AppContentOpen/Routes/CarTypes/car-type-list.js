...
const CarTypeTable = () => {
  ...
  const deleteRow = async (id, image) => {
    // console.log(`Item ID: ${id}`);

    //! Calling Api for deleting Row By ID...
    let apiUrl = `${host[0].hostUrl}/api/delete/data/carTypes/row/byId`;
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const filterData = data.filter(({ id, carType }) => (carType.toLowerCase().includes(inputValue) || id.toString().includes(inputValue)));
    ...
    <>
      ...
      >
        <div className="btn btn-primary" style={{ marginBottom: 10 }}>
          <Link to={"/carTypeForm"} style={{ color: "white" }}>
            Add Car New Type
          </Link>
        </div>
      </div>
          ...
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header border-bottom" style={{ display: 'flex', justifyContent: 'space-between' }}>
                <h3 className="card-title">Car Types List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search Car Type..." type="search" style={{ paddingRight: 40 }} />
                          ...
                          currentData.length > 0 ?
                            currentData.map((item, index) => {
                              return (
                                ...
                                <td data-field="name">{item.carType}</td>
                                <td data-field="age">
                                  <img
                                    src={`${host[0].hostUrl}/images/type-images/${item.image}`}
                                    alt={item.image}
                                    onError={(e) => {
                                ...
                                <td data-field="age">{item.abbrevation}</td>
                                <td data-field="age">{item.category}</td>
