...
const CitiesTable = () => {
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const filterData = data.filter(({ id, cityName }) => (cityName.toLowerCase().includes(inputValue) || id.toString().includes(inputValue)));
                ...
                <h3 className="card-title">City List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search Cities..." type="search" style={{ paddingRight: 40 }} />
                    ...
                    <tbody>
                      {
                        searchValue.trim() == "" ?
                            ...
                            currentData.map((item, index) => {
                              return (
                                <tr data-id="1" key={index}>
                                  ...
                                  <td data-field="age">
                                    <img
                                      src={`${host[0].hostUrl}/images/cities-images/${item.pictureUrl}`}
                                      alt={item.pictureUrl}
                                      // onError={(e)=>{e.target.src = imageErrorIcon}}
                                      onError={(e) => { e.target.src = e.target.src + '.png' }}
                                      height={50}
                                      width={100}
                          ...
                          filterData.length > 0 ?
                            ...
                            :
                            <tr data-id="0">
                              <td data-field="id" colSpan={5} className="text-center p-5"><h4>No Record Found !</h4></td>
                            </tr>
                      }
                    </tbody>
                  </table>
