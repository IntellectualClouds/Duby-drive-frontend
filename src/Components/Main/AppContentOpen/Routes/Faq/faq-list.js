...
const FaqTable = () => {
  ...
  //! Fetching FAQ List Api...
  const fetchingApi = async () => {
    let apiUrl = `${host[0].hostUrl}/api/get/data/fAQ`;
  ...
  const deleteRow = async (id) => {
    ...
    let apiUrl = `${host[0].hostUrl}/api/delete/data/fAQ/row/byId`;
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const dealer = vendersData.find((data) => (data.companyName.toLowerCase().includes(inputValue)));

      const filterData = data.filter(({ id, faqType }) => (id.toString().includes(inputValue) || faqType.toLowerCase().includes(inputValue)));
      if (filterData.length > 0) {
        setFilterData(filterData);
      }
      else if (dealer) {
        const filteredByDealer = data.filter(({ vendorId }) => (vendorId == dealer.id));
    ...
    <>
      ...
      >
        <div className="btn btn-primary" style={{ marginBottom: 10 }}>
          <Link to={"/faqForm"} style={{ color: "white" }}>
            Add New Faq
          </Link>
        </div>
      </div>
                ...
                <h3 className="card-title">Faq List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search FAQ'S..." type="search" style={{ paddingRight: 40 }} />
                        ...
                        searchValue.trim() == "" ?
                          currentData.length > 0 ?
                            currentData.map((item, index) => {
                              ...
                              <td data-field="name">{item.faqHeading}</td>
                              <td
                                data-field="age"
                                style={{ backgroundColor: "none", width: 550 }}
                              >
                                {item.faqDescription}
                              </td>
                              <td data-field="gender" style={{ width: 100 }}>
                                <span
                              ...
                              <td data-field="gender" >
                                ...
                                >
                                  {/* {item.vendorId} */}
                                  {
                                    item.faqType !== 'Duby Drive' ?
                                      vendersData.length > 0 ?
                                        vendersData.map((data, key) => {
                                          return (
                                            <React.Fragment key={key}>{item.vendorId == data.id ? data.companyName : ''}</React.Fragment>
                                          )
                                        })
                                        :
                                        item.vendorId
                          ...
                          filterData.length > 0 ?
                            ...
                            :
                            <tr data-id="0">
                              <td data-field="id" colSpan={7} className="text-center p-5"><h4>No Record Found !</h4></td>
                            </tr>
                      }
                    </tbody>
                  </table>
