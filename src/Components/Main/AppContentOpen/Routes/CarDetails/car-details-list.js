...
const CarDetailsTable = () => {
  ...
  const deleteRow = async (id, featuredImg, photosArray, videoName) => {
      ...
      if (result.isConfirmed) {
        try {
          let response = await axios({
            method: "DELETE",
            url: apiUrl,
            data: { id, featuredImg, photosArray, videoName, videoStatus },
          });
          // console.log(response);
          if (response.status == 200) {
            fetchingApi();
  ...
  const fetchingCarBrandsApi = async () => {
    let apiUrl = `${host[0].hostUrl}/api/get/data/carBrands`;
  ...
  const fetchingCarTypesApi = async () => {
    let apiUrl = `${host[0].hostUrl}/api/get/data/carTypes`;
  ...
  const onChangeHandler = (e) => {
    ...
    // else if (inputValue.trim() == 'cars' || inputValue.trim() == 'car') {
    //   setFilterData([]);
    // }
    else {
      const brand = brandsData.find((data) => (data.brandName.toLowerCase().includes(inputValue)));
      const type = carTypeData.find((data) => (data.carType.toLowerCase().includes(inputValue)));
      const dealer = dealerData.find((data) => (data.companyName.toLowerCase().includes(inputValue)));

      const filterData = data.filter(({ carModelNo, id, carFuelType }) => (carModelNo.toLowerCase().includes(inputValue) || id.toString().includes(inputValue) || carFuelType.toLowerCase().includes(inputValue)));
      if (filterData.length > 0) {
        setFilterData(filterData);
      }
      else if (brand) {
        const filterCarByBrand = data.filter(({ carBrandId }) => (carBrandId == brand.id));
        if (filterCarByBrand.length > 0) {
          setFilterData(filterCarByBrand);
        }
        else {
          setFilterData([]);
        }
        ...
        const filterCarByType = data.filter(({ carTypeId }) => (carTypeId == type.id));
        if (filterCarByType.length > 0) {
          setFilterData(filterCarByType);
        }
        else {
          setFilterData([]);
        }
        ...
        const filterCarByDealer = data.filter(({ dealerCompanyId }) => (dealerCompanyId == dealer.id));
        if (filterCarByDealer.length > 0) {
          setFilterData(filterCarByDealer);
        }
        else {
          setFilterData([]);
        }
                ...
                <h3 className="card-title">Car Details List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search Vehicle..." type="search" style={{ paddingRight: 40 }} />
                                ...
                                <tr data-id="1" key={index}>
                                  ...
                                  </td>
                                  {
                                    item.carPhotosArray !== null ?
                                      ...
                                      >
                                        <img
                                          src={`${host[0].hostUrl}/images/cars-images/${item.featuredImg}`}
                                          alt={item.carPhotosArray[0]}
                                          onError={(e) => {
                                  ...
                                  >
                                    {brandsData.length > 0 ?
                                      brandsData.map((brand, key) => {
                                        return (
                                          brand.id == item.carBrandId ? brand.brandName : ''
                                        );
                                      })
                                      :
                                      `#${item.carBrandId}`
                                  ...
                                  >
                                    {dealerData.length > 0 ?
                                      dealerData.map((dealer, key) => {
                                        return (
                                          dealer.id == item.dealerCompanyId ? dealer.companyName : ''
                                        );
                                      })
                                      :
                                  ...
                                  >
                                    {carTypeData.length > 0 ?
                                      carTypeData.map((carType, key) => {
                                        return (
                                          carType.id == item.carTypeId ? carType.carType : ''
                                        );
                                      })
                                      :
                                      `#${item.carTypeId}`
          ...
          >
              ...
              <h6 className="modal-title">
                {
                  modalData !== null ?
                    <div>
                      {
                        modalData.category.verified == 'verified' ?
                          <span className="mb-0 badge bg-primary me-1">Verified</span>
                          :
                          ''
                      }
                      {
                        modalData.category.featured == 'featured' ?
                          <span className="mb-0 badge bg-warning me-1">Featured</span>
                          :
                          ''
                      }
                      {
                        modalData.category.premium == 'premium' ?
                          <span className="mb-0 badge bg-dark">Premium</span>
                          :
                          ''
                      }
                    </div>
            ...
            {modalStatus == true ? (
              <div className="modal-body">
                {
                  modalData.carPhotosArray !== null ?
                    <img
                      src={`${host[0].hostUrl}/images/cars-images/${modalData.featuredImg}`}
                      alt={modalData.carPhotosArray[0]}
                      onError={(e) => {
                        e.target.src = imageErrorIcon;
                      }}
                      height={70}
                ...
                >
                  ...
                  <p>
                    {dealerData.length > 0 ?
                      dealerData.map((dealer, key) => {
                        return (
                          dealer.id == modalData.dealerCompanyId ? dealer.companyName : ''
                        );
                      })
                      :
                ...
                <div style={{ backgroundColor: "none", marginTop: 20 }}>
                    ...
                    >
                      ...
                      <p>
                        {brandsData.length > 0 ?
                          brandsData.map((brand, key) => {
                            return (
                              brand.id == modalData.carBrandId ? brand.brandName : ''
                            );
                          })
                          :
                          `#${modalData.carBrandId}`
                        }
                      </p>
                    </label>
                    <label
                    ...
                    >
                      ...
                      <p>
                        {carTypeData.length > 0 ?
                          carTypeData.map((carType, key) => {
                            return (
                              carType.id == modalData.carTypeId ? carType.carType : ''
                            );
                          })
                          :
                          `#${modalData.carTypeId}`
                  ...
                  >
                    ...
                    >
                      <b> Exterior Color:</b>
                      <p>{modalData.carExteriorColor ? modalData.carExteriorColor : '-'}</p>
                    </label>
                    <label
                      style={{
                        display: "flex",
                    ...
                    >
                      <b> Interior Color:</b>
                      <p>{modalData.carInteriorColor ? modalData.carInteriorColor : '-'}</p>
                    </label>
                    <label
                      style={{
                        display: "flex",
                  ...
                  >
                    ...
                    >
                      <b> Discount Offer:</b>
                      <p>{modalData.discountOffer}% Discount</p>
                    </label>
                    <label
                      style={{
                        display: "flex",
                  ...
                  >
                    ...
                    <p dangerouslySetInnerHTML={{ __html: modalData.description }}></p>
                  </label>
                </div>
              </div>
            ) : (
            ...
            >
              ...
              >
                {photosModalData.length > 0 ? (
                  <img
                    src={`${host[0].hostUrl}/images/cars-images/${photosModalData[modalImageNumber]}`}
