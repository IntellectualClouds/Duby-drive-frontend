...
const CarFeatureTable = () => {
  ...
  const fetchingApi = async () => {
    let apiUrl = `${host[0].hostUrl}/api/get/data/carFeatures`;
  ...
  const deleteRow = async (id) => {
    ...
    let apiUrl = `${host[0].hostUrl}/api/delete/data/carFeatures/row/byId`;
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const filterData = data.filter(({ id, carFeature }) => (carFeature.toLowerCase().includes(inputValue) || id.toString().includes(inputValue)));
    ...
    <>
      ...
      >
        <div className="btn btn-primary" style={{ marginBottom: 10 }}>
          <Link to={"/carFeaturesForm"} style={{ color: "white" }}>
            Add New Car Feature
          </Link>
        </div>
      </div>
          ...
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header border-bottom" style={{ display: 'flex', justifyContent: 'space-between' }}>
                <h3 className="card-title">Car Features List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search Car Feature..." type="search" style={{ paddingRight: 40 }} />
                    ...
                    <tbody>
                      {
                        searchValue.trim() == "" ?
                            ...
                            currentData.map((item, index) => {
                              return (
                                <tr data-id="1" key={index}>
                                  ...
                                  <td data-field="name">{item.carFeature}</td>
                          ...
                          filterData.length > 0 ?
                            ...
                            :
                            <tr data-id="0">
                              <td data-field="id" colSpan={4} className="text-center p-5"><h4>No Record Found !</h4></td>
                            </tr>
                      }
                    </tbody>
                  </table>
...
export default CarFeatureTable;
