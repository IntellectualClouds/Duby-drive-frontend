...
const BannerTable = () => {
    ...
    const fetchingApi = async () => {
        let apiUrl = `${host[0].hostUrl}/api/get/data/banner`;
    ...
    const deleteRow = async (id) => {
        ...
        let apiUrl = `${host[0].hostUrl}/api/delete/data/banner/row/by/Id`;
    ...
    const onChangeHandler = (e) => {
        ...
        if (inputValue.trim() == '') {
            ...
            const filterData = data.filter(({ id, bannerPage, bannerNumber }) => (bannerPage.toLowerCase().includes(inputValue) || id.toString().includes(inputValue) || bannerNumber.toLowerCase().includes(inputValue)));
        ...
        <>
            ...
            >
                <div className="btn btn-primary" style={{ marginBottom: 10 }}>
                    <Link to={"/bannerForm"} style={{ color: "white" }}>
                        Add New Banner
                    </Link>
                </div>
            </div>
                                ...
                                <h3 className="card-title">Banner List</h3>
                                {
                                    data.length > 0 ?
                                        <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                                            <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search Banner..." type="search" style={{ paddingRight: 40 }} />
                                        ...
                                        <tbody>
                                            {
                                                searchValue.trim() == "" ?
                                                        ...
                                                        currentData.map((item, index) => {
                                                            return (
                                                                <tr data-id="1" key={index}>
                                                                    ...
                                                                    <td data-field="name">{item.bannerPage}</td>
                                                                    <td data-field="age">{item.bannerNumber}</td>
                                                                    <td onClick={() => { setModalStatus(true); setModalData(item.bannerHTML) }} style={{ color: '#8fbd56' }} className="banner-open-link">{item.bannerHTML == "" ? '-' : 'Click to view Banner HTML'}</td>
                                                    ...
                                                    filterData.length > 0 ?
                                                        ...
                                                        :
                                                        <tr data-id="0">
                                                            <td data-field="id" colSpan={6} className="text-center p-5"><h4>No Record Found !</h4></td>
                                                        </tr>
                                            }
                                        </tbody>
                                    </table>
                ...
                <div className="modal-dialog modal-lg " role="document">
                    ...
                    >
                        ...
                        <div className="modal-body" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                            <p style={{ backgroundColor: 'none', height: '100%', width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }} dangerouslySetInnerHTML={{ __html: modalData }}></p>
                            <div style={{ display: 'flex', flexWrap: 'wrap' }}>
                                <pre style={{ textWrap: 'wrap', width: 750 }}>{modalData}</pre>
                            </div>
                        </div>
                        <div
