...
const CarBrandTable = () => {
  ...
  const deleteRow = async (id, logo) => {
    ...
    let apiUrl = `${host[0].hostUrl}/api/delete/data/carBrands/row/byId`;
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const filterData = data.filter(({ id,brandName }) => (brandName.toLowerCase().includes(inputValue) || id.toString().includes(inputValue)));
    ...
    <>
      ...
      >
        <div className="btn btn-primary" style={{ marginBottom: 10 }}>
          <Link to={"/carBrandForm"} style={{ color: "white" }}>
            Add New Car Brand
          </Link>
        </div>
      </div>
          ...
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header border-bottom" style={{ display: 'flex', justifyContent: 'space-between' }}>
                <h3 className="card-title">Car Brands List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search Car Brand..." type="search" style={{ paddingRight: 40 }} />
                ...
                <div className="table-responsive export-table">
                  ...
                  >
                    <thead>
                      <tr>
                        <th>ID</th>
                        <th style={{width:170}}>Brand Name</th>
                        <th style={{width:150}}>Logo</th>
                        <th>Description</th>
                        <th>Status</th>
                        <th style={{width:120}}>Desc Status</th>
                        <th>Buttons</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        searchValue.trim() == "" ?
                          currentData.length > 0 ?
                            currentData.map((item, index) => {
                              ...
                              <td data-field="name">{item.brandName}</td>
                              {/* <td data-field="age">{item.logo}</td> */}
                              <td data-field="age" style={{display:'flex',justifyContent:'center'}}>
                                <img
                                  src={`${host[0].hostUrl}/images/car-brands-logo/${item.logo}`}
                                  alt={item.logo}
                                  onError={(e) => {
                              ...
                              <td data-field="name">{item.brandDescription}</td>
