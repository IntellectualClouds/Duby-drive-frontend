...
const AreaTable = () => {
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const filterData = data.filter(({ id, areaName }) => (areaName.toLowerCase().includes(inputValue) || id.toString().includes(inputValue)));
                ...
                <h3 className="card-title">Area List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search City Area..." type="search" style={{ paddingRight: 40 }} />
                            ...
                            currentData.map((item, index) => {
                              return (
                                <tr data-id="1" key={index}>
                                  ...
                                  <td data-field="name">{item.cityId}</td>
                                  <td data-field="name">
