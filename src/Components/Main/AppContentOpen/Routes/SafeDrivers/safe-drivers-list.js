...
const SafeDriversTable = () => {
  ...
  //! Fetching Safe Drivers List Api...
  const fetchingApi = async () => {
    let apiUrl = `${host[0].hostUrl}/api/get/data/safeDrivers`;
  ...
  const deleteRow = async (id) => {
    ...
    let apiUrl = `${host[0].hostUrl}/api/delete/data/safeDrivers/row/byId`;
  ...
  const onChangeHandler = (e) => {
    ...
    if (inputValue.trim() == '') {
      ...
      const filterData = data.filter(({ id, servicePeriod, serviceType, carWithDriver }) => (servicePeriod.toLowerCase().includes(inputValue) || id.toString().includes(inputValue) || serviceType.toLowerCase().includes(inputValue) || carWithDriver.toLowerCase().includes(inputValue)));
    ...
    <>
      ...
      >
        <div className="btn btn-primary" style={{ marginBottom: 10 }}>
          <Link to={"/safeDriversForm"} style={{ color: "white" }}>
          ...
          <div className="col-lg-12">
            <div className="card">
              <div className="card-header border-bottom" style={{ display: 'flex', justifyContent: 'space-between' }}>
                <h3 className="card-title">Driver Services List</h3>
                {
                  data.length > 0 ?
                    <div className="main-header-center ms-3 d-none d-xl-block" style={{ position: 'relative' }}>
                      <input autoCorrect="on" className="form-control" onChange={onChangeHandler} placeholder="Search Driver..." type="search" style={{ paddingRight: 40 }} />
                            ...
                            currentData.map((item, index) => {
                              return (
                                <tr data-id="1" key={index}>
                                  ...
                                  <td data-field="name">{item.servicePeriod}</td>
                                  <td data-field="name">{item.serviceType}</td>
                                  <td data-field="name">{item.carWithDriver}</td>
                                  <td data-field="name">{
                                    dealerData.length > 0 ?
                                  ...
                                  <td data-field="name" style={{ width: 350 }} dangerouslySetInnerHTML={{ __html: item.driverDescription }}></td>
                                  <td data-field="name">{item.servicePrice} AED</td>
...
export default SafeDriversTable;
